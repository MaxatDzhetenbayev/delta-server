...
export class CardsService {
  ...
  async getAllCards(question: string, tag: string) {
    ...
    try {
      ...
      if (!cards.length) {
        throw new HttpException(`Карточки не найдены`, HttpStatus.NOT_FOUND);
      }
  ...
  async getAllCardsByUserId(id: number, question: string, tag: string) {
    ...
    try {
      ...
      if (!cards.length) {
        throw new HttpException(`Карточки пользователя с id ${id} не найдены`, HttpStatus.NOT_FOUND);
      }
  ...
  async deleteOneCardById(id: string) {
    try {
      const findedCard = await this.cardsRepository.findOne({
        where: {
          id,
        },
      });
      ...
      return {
        status: 'success',
        message: `Карточка с id ${findedCard.id} удалена`,
      };
    } catch (error) {
      console.log(error);
      throw new HttpException(`${error}`, HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  async updateCardById(id: string, newBody: CardUpdateDto) {
    try {
      ...
      return {
        status: 'success',
        message: `Карточка с id ${findedCard.id} изменена`,
      };
    } catch (error) {
      console.log(error);
      throw new HttpException(`${error}`, HttpStatus.INTERNAL_SERVER_ERROR);
